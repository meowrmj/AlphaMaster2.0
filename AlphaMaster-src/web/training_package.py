...
def _artifact_tag(symbol: str, timeframe: str | None = None) -> str:
    ...
    if tf:
        tag = f"{tag}_{re.sub(r'[^A-Za-z0-9._-]+', '_', tf)}"
    return tag


def _history_path(symbol: str, timeframe: str | None = None) -> Path:
    return PROJECT_ROOT / f"training_history_{_artifact_tag(symbol, timeframe)}.json"
...
def build_training_export_zip(symbol: str, timeframe: str | None = None) -> tuple[bytes, str]:
    ...
    history_path = _history_path(symbol, timeframe)

    step = int(re.search(r"_step_(\d+)\.pt$", latest.name).group(1))
    safe = _artifact_tag(symbol, timeframe)
    ...
    with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as zf:
        ...
        if strategy:
            strat_name = f"strategies/best_{safe}.json"
        ...
        if history_path.exists():
            hist_name = f"training_history_{safe}.json"
