...
def resolve_strategy_file(
    ...
    if saved_path:
        p = Path(saved_path)
        if p.exists():
            if train_symbol or train_timeframe:
                try:
                    data = json.loads(p.read_text(encoding="utf-8"))
                except (json.JSONDecodeError, OSError):
                    data = {}
                saved_symbol = data.get("symbol") if isinstance(data, dict) else None
                saved_timeframe = data.get("timeframe") if isinstance(data, dict) else None
                symbol_ok = not train_symbol or saved_symbol == train_symbol
                timeframe_ok = not train_timeframe or saved_timeframe == train_timeframe
                if symbol_ok and timeframe_ok:
...
def sync_best_strategy_for_symbol(
    ...
    *,
    ...
    if not target_timeframe and data_file_hint:
        try:
            target_timeframe = inspect_parquet_file(str(Path(data_file_hint).resolve())).get("timeframe")
        except Exception:
    ...
    for path in STRATEGIES_DIR.glob(f"strategy_{safe}_*.json"):
        ...
        if not formula or score is None:
            continue
        if target_timeframe and data.get("timeframe") and data.get("timeframe") != target_timeframe:
            continue
    ...
    if target_timeframe:
        payload["timeframe"] = target_timeframe
