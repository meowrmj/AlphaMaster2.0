...
def _save(symbol: str, timeframe: str | None, data: dict[str, Any]) -> None:
    path = _stats_path(symbol, timeframe)
...
def _log_timeframe(path: Path, symbol: str) -> str | None:
    try:
        text = path.read_text(encoding="utf-8", errors="ignore")
    except OSError:
        return None
    safe = re.escape(symbol.replace(".", "_"))
    m = re.search(rf"({safe}_[A-Za-z0-9]+\.parquet)", text)
    if not m:
        return None
    try:
        parsed_symbol, timeframe = parse_parquet_filename(m.group(1))
    except ValueError:
        return None
    return timeframe if parsed_symbol == symbol else None
...
def _backfill_from_logs(
    ...
    backfill_key = f"{symbol}:{timeframe or ''}"
    if backfill_key in _backfilled:
        return data
    _backfilled.add(backfill_key)
    ...
    for path in sorted(LOG_DIR.glob(f"train_{safe}_*.log")):
        ...
        if not m:
            continue
        detected_timeframe = _log_timeframe(path, symbol)
        if timeframe and detected_timeframe != timeframe:
            continue
        try:
            st = path.stat()
        except OSError:
...
def record_training_session(
    *,
    ...
    with _lock:
        data = _backfill_from_logs(symbol, timeframe, _load(symbol, timeframe))
...
def get_training_time_summary(
    ...
    *,
    ...
    if (
        ...
        and (not timeframe or str(job.get("timeframe") or "") == timeframe)
    ):
