...
def main():
    ...
    strategy_file = None
    data_file_arg = None
    strategy_oos_start_bar = None
    commission_pct = DEFAULT_COMMISSION_PCT
    slippage_pct = DEFAULT_SLIPPAGE_PCT
    oos_ratio = DEFAULT_OOS_RATIO
    full_sample = "--full-sample" in sys.argv
    for i, arg in enumerate(sys.argv):
        ...
        elif arg == "--oos-ratio" and i + 1 < len(sys.argv):
            oos_ratio = float(sys.argv[i + 1])

    if commission_pct < 0 or slippage_pct < 0:
        print("[ERROR] 手续费/滑点不能为负"); sys.exit(1)
    if not 0.0 < oos_ratio < 1.0:
        print("[ERROR] --oos-ratio 必须在 0 和 1 之间"); sys.exit(1)
    ...
    if strategy_file:
        ...
        strategy_oos_start_bar = data.get("oos_start_bar")
        sym = data.get("symbol")
        if not sym:
        ...
        for sym in Config.SYMBOLS:
            ...
            if not strategy_data_file and data.get("data_file"):
                strategy_data_file = data.get("data_file")
            if strategy_oos_start_bar is None and data.get("oos_start_bar") is not None:
                strategy_oos_start_bar = data.get("oos_start_bar")
            sc = data.get("best_score", "N/A")
    ...
    eval_label = "全样本"
    if not full_sample:
        if strategy_oos_start_bar is not None:
            eval_start = int(strategy_oos_start_bar)
            eval_label = "策略预留样本外"
        else:
            eval_start = int(T * (1.0 - oos_ratio))
            eval_label = f"样本外后 {oos_ratio:.0%}"
        eval_start = max(1, min(T - 2, eval_start))
    eval_bars = T - eval_start
    print(f"  回测区间: {eval_label}  bars=[{eval_start},{T})  共 {eval_bars} 根")
    if not full_sample:
        print("  [防泄露] 默认只统计样本外区间；如需复盘整段历史，请显式加 --full-sample")
    ...
    for i, sym in enumerate(syms):
        ...
        feat_i    = feat[i:i+1, :, eval_start:]
        raw_i     = {k: v[i:i+1, eval_start:] for k, v in raw_dict.items()}
    ...
    Path(OUTPUT_DIR).mkdir(parents=True, exist_ok=True)
    if results_map:
        times_np = times_all[0, eval_start:].numpy() if times_all is not None else None
    ...
    report = {
        ...
        "evaluation": {
            "sample": "full" if full_sample else "out_of_sample",
            "oos_ratio": None if full_sample else oos_ratio,
            "start_bar": eval_start,
            "end_bar": T,
            "bars": eval_bars,
            "note": "默认只统计最后一段样本外收益，避免把训练见过的历史当测试成绩。",
        },
