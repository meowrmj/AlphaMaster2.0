...
at::Tensor fused_unary_binary_branch_cuda(at::Tensor a, at::Tensor b, at::Tensor c, at::Tensor d, int64_t unary_op_id, int64_t binary_op_id, int64_t branch_op_id);
at::Tensor fused_unary_unary_branch_cuda(at::Tensor a, at::Tensor b, at::Tensor c, int64_t first_unary_op_id, int64_t second_unary_op_id, int64_t branch_op_id);
at::Tensor fused_rolling_binary_branch_cuda(at::Tensor a, at::Tensor b, at::Tensor c, at::Tensor d, int64_t rolling_op_id, int64_t binary_op_id, int64_t branch_op_id);
...
at::Tensor fused_unary_binary_branch(at::Tensor a, at::Tensor b, at::Tensor c, at::Tensor d, int64_t unary_op_id, int64_t binary_op_id, int64_t branch_op_id) {
  TORCH_CHECK(a.is_cuda(), "a must be a CUDA tensor");
  TORCH_CHECK(b.is_cuda(), "b must be a CUDA tensor");
  TORCH_CHECK(c.is_cuda(), "c must be a CUDA tensor");
  TORCH_CHECK(d.is_cuda(), "d must be a CUDA tensor");
  TORCH_CHECK(a.sizes() == b.sizes() && a.sizes() == c.sizes() && a.sizes() == d.sizes(), "all tensors must have the same shape");
  return fused_unary_binary_branch_cuda(a.contiguous(), b.contiguous(), c.contiguous(), d.contiguous(), unary_op_id, binary_op_id, branch_op_id);
}

at::Tensor fused_unary_unary_branch(at::Tensor a, at::Tensor b, at::Tensor c, int64_t first_unary_op_id, int64_t second_unary_op_id, int64_t branch_op_id) {
  TORCH_CHECK(a.is_cuda(), "a must be a CUDA tensor");
  TORCH_CHECK(b.is_cuda(), "b must be a CUDA tensor");
  TORCH_CHECK(c.is_cuda(), "c must be a CUDA tensor");
  TORCH_CHECK(a.dim() == 3, "a must be [B,N,T]");
  TORCH_CHECK(a.sizes() == b.sizes() && a.sizes() == c.sizes(), "all tensors must have the same shape");
  return fused_unary_unary_branch_cuda(a.contiguous(), b.contiguous(), c.contiguous(), first_unary_op_id, second_unary_op_id, branch_op_id);
}

at::Tensor fused_rolling_binary_branch(at::Tensor a, at::Tensor b, at::Tensor c, at::Tensor d, int64_t rolling_op_id, int64_t binary_op_id, int64_t branch_op_id) {
  TORCH_CHECK(a.is_cuda(), "a must be a CUDA tensor");
  TORCH_CHECK(b.is_cuda(), "b must be a CUDA tensor");
  TORCH_CHECK(c.is_cuda(), "c must be a CUDA tensor");
  TORCH_CHECK(d.is_cuda(), "d must be a CUDA tensor");
  TORCH_CHECK(a.dim() == 3, "a must be [B,N,T]");
  TORCH_CHECK(a.sizes() == b.sizes() && a.sizes() == c.sizes() && a.sizes() == d.sizes(), "all tensors must have the same shape");
  return fused_rolling_binary_branch_cuda(a.contiguous(), b.contiguous(), c.contiguous(), d.contiguous(), rolling_op_id, binary_op_id, branch_op_id);
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  m.def("fused_unary_binary_branch", &fused_unary_binary_branch, "AlphaMaster fused unary+binary+branch CUDA ops");
  m.def("fused_unary_unary_branch", &fused_unary_unary_branch, "AlphaMaster fused unary+unary+branch CUDA ops");
  m.def("fused_rolling_binary_branch", &fused_rolling_binary_branch, "AlphaMaster fused rolling+binary+branch CUDA ops");
